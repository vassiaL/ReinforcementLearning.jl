...
function RIntegratorStateActionReward(; ns = 10, na = 4)
    Nsa = zeros(Int, na, ns)
    Rsum = zeros(na, ns)
    R = zeros(na, ns)
    RIntegratorStateActionReward(ns, na, Nsa, Rsum, R)
end
...
function RIntegratorNextStateReward(; ns = 10)
    Ns = zeros(Int, ns)
    Rsum = zeros(ns)
    R = zeros(ns)
    RIntegratorNextStateReward(ns, Ns, Rsum, R)
end
...
function RLeakyIntegratorStateActionReward(; ns = 10, na = 4, etaleak = 0.9)
    Nsa = zeros(na, ns)
    Rsum = zeros(na, ns)
    R = zeros(na, ns)
    RLeakyIntegratorStateActionReward(ns, na, etaleak, Nsa, Rsum, R)
end
